...
// Wide authored canvases should use their available horizontal space. The
// previous fixed 640px lane left a large empty band on Russian business maps
// and forced readable action names into unexplained abbreviations. Preserve
// the compact 720px layout, but distribute the six workflow columns across a
// wider explicit viewBox.
const requestedViewBox = workflow.meta?.viewBox;
if (Array.isArray(requestedViewBox) && requestedViewBox[0] > 720) {
  layout.laneW = Math.max(layout.laneW, requestedViewBox[0] - 80);
  const firstColX = layout.laneX + 82;
  const lastColX = layout.laneX + layout.laneW - 82;
  const step = (lastColX - firstColX) / (layout.colXs.length - 1);
  layout.colXs = layout.colXs.map((_x, index) => Math.round(firstColX + step * index));
}
...
const autoHeight = layout.laneY
  ...
  + 124;
const viewBox = requestedViewBox || [720, autoHeight];
...
function nodeContext(node) {
  ...
  return [laneLabels.get(node.lane), group?.label, phase?.label].filter(Boolean).join(' › ') || 'Шаг процесса';
}
...
function renderLane(lane, index) {
  ...
  return `        <rect data-graph-role="structural-frame" data-composition-frame-kind="lane" data-composition-frame-id="lane-${index}" x="${layout.laneX}" y="${y}" width="${layout.laneW}" height="${layout.laneH}" rx="10" class="c-lane" stroke-width="1"/>${exception}
        <text x="${layout.laneX + 14}" y="${y + 22}" class="${labelClass}" font-size="10" font-weight="600">${esc(lane.label)}</text>`;
}
