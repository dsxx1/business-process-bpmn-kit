...
**Графический BPMN-редактор:** роли, действия, развилки, результаты и связи
редактируются в одном окне.

![Графический BPMN-редактор в локальной Студии](docs/assets/bpmn-studio.png)

**Archify внутри Студии:** та же модель превращается в крупную читаемую карту
для обсуждения процесса с сотрудниками и руководителем.

![Карта Archify внутри локальной BPMN-студии](docs/assets/bpmn-studio-archify.png)
...
Подробнее: [как настроить переход](docs/ПЕРЕХОДЫ-МЕЖДУ-BPMN-ПРОЦЕССАМИ.md) и
[какой вид связи выбрать](docs/ПЕРЕХОДЫ-МЕЖДУ-ПРОЦЕССАМИ.md).
...
| [BPMN-схема](processes/skupka-zolota/bpmn/derived/process.svg) | Просмотреть полную логику прямо в браузере |
| [Исходный BPMN 2.0](processes/skupka-zolota/bpmn/process.bpmn) | Открыть в любом совместимом редакторе или движке |

Этот пример показывает механику комплекта, а не утверждённый регламент.
Его бизнес-статус — `pending_human_decision`, вопросы владельцу в
`questions.json` остаются открытыми. Так и должно выглядеть корректно
оформленное описание процесса до решения уполномоченного владельца.
...
Та же проверка запускается автоматически: в GitHub Actions
([.github/workflows/verify.yml](.github/workflows/verify.yml)) и в GitLab CI
([.gitlab-ci.yml](.gitlab-ci.yml)) на каждый push и pull/merge request.

## Лицензия

Проект распространяется по лицензии [MIT](LICENSE). Встроенные компоненты
`vendor/bpmn-js` и `vendor/archify` сохраняют собственные лицензии MIT —
файлы лицензий и происхождение указаны в `PROVENANCE.md` каждого каталога.

Автоматическая проверка и лицензия не заменяют бизнес-приёмку: содержательное
решение по процессу принимает уполномоченный владелец.
